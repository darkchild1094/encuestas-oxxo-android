...
import com.kernel94.pulsoti.data.remote.dto.*
...
class EncuestaRepository(
    ...
    suspend fun guardarYIntentarSincronizar(
        ...
        // Locale.US fijo: el formato no lleva nombres de mes, pero en
        // locales con digitos propios (ar, fa, algunos hi) format()
        // produciria numeros que MariaDB no parsea. US = ASCII siempre.
                ...
                if (fechaUltima != null) {
                    ...
                    if (minutos < 30) {
                        ...
                        if (folioUltimo.isEmpty() && folioActual.isNotEmpty()) {
                            ...
                            encuestaDao.reemplazarPrevia(ultima)
        ...
        // Cabecera + respuestas en una sola transaccion (ver EncuestaDao).
        encuestaDao.guardarEncuestaCompleta(encuesta, respuestas)
